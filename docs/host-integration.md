...
4. Declare `truffler` on items and messages: `tenant`, `reads`, the labels, `keyword` over the plaintext columns, and `order` and `arrived_at`. Declare `surface :feed, explicit_action: :enter` for the feed search box.
   - happyhappy's Classification already answers sentiment, anger, category, product, actionability, and author role. Declare those with `from:` (for example `label :sentiment, :choice, options: [...], from: ->(item) { item.classification&.sentiment }`) so Jev is never asked them again and they cost nothing. Only questions Classification does not answer should use `question:`.
   - Classification runs in a job after insert, so call `item.truffler_refresh_labels!` at the end of that job. If the answers live in columns on the item, list them in `watch:` instead. Bump `version:` when the classifier changes, then run the backfill, which rewrites supplied labels at no cost.
...
6. In the feed controller, call `Model.truffler(...)` on every keystroke, and pass `result.records`, `chips`, `invite_row`, `encoding_status`, and `watermark` as props. On the explicit action, call `jev_smart_search` and redirect with `run_id`. Load the run with `Truffler::SmartSearch.find(run_id, user:, tenant:)` (a run owned by another searcher or tenant reads as expired) and render `smart: -> { run&.to_h }` as a lazy prop.
