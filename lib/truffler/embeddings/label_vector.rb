module Truffler
  module Embeddings
    ...
    # rebuilds them without asking Jev.
    class LabelVector
      attr_reader :model
      ...
      def keys(tenant_key = nil)
        definition.labels.values.flat_map { |label| label.storage_keys(tenant_key) }.sort
      end
      ...
      def write(record_ids, tenant_key:)
        ...
        version = definition.vocabulary.version(tenant_key: tenant_key)
      ...
      def rebuild(tenant_key: :all, batch_size: 500)
        ...
        tenants.sum do |tenant|
          version = definition.vocabulary.version(tenant_key: tenant)
      ...
      def read(record)
        ...
        return unless row&.label_vector && row.label_vocabulary_version == definition.vocabulary.version(tenant_key: tenant_key)
